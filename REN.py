...
class RegionEnsemble(nn.Module):
    ...
    def forward(self, x):
        midpoint = self.feat_size // 2
        quarterpoint1 = midpoint // 2
        quarterpoint2 = quarterpoint1 + midpoint
        regions = []
        ensemble = []

        # 4 corners
        regions += [x[:, :, :midpoint, :midpoint].clone(), x[:, :, :midpoint, midpoint:].clone(),
                    x[:, :, midpoint:, :midpoint].clone(), x[:, :, midpoint:, midpoint:].clone()]
        # 4 overlapping centers

        regions += [x[:, :, quarterpoint1:quarterpoint2, :midpoint].clone(),
                    x[:, :, quarterpoint1:quarterpoint2, midpoint:].clone(),
                    x[:, :, :midpoint, quarterpoint1:quarterpoint2].clone(),
                    x[:, :, midpoint:, quarterpoint1:quarterpoint2].clone()]
        # middle center
