...
msra_path = pathlib.Path("/home/george/workspace/data/msra15")
...
class MSRADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        path = msra_path / f"P{person}/{name}/{file}_depth.bin"
        depth = read_depth_from_bin(path)
...
def read_joints(persons=None, poses=None):
    if persons is None:
        persons = [0, 1, 2, 3, 4, 5, 6, 7]
    if poses is None:
    ...
    for person in persons:
        for pose in poses:
            path = msra_path / f"P{person}/{pose}/joint.txt"
            with path.open() as f:
